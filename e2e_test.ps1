...
# 1. Check services
Write-Host ">>> [1/5] Checking port 8080 (Java) and 8000 (Python)..." -ForegroundColor Yellow

$javaIsUp = (Test-NetConnection -ComputerName 127.0.0.1 -Port 8080 -InformationLevel Quiet)
if (-not $javaIsUp) {
    Write-Host "ERROR: Java Spring Boot service (Port 8080) is not running!" -ForegroundColor Red
    exit 1
}

$pythonIsUp = (Test-NetConnection -ComputerName 127.0.0.1 -Port 8000 -InformationLevel Quiet)
if (-not $pythonIsUp) {
    Write-Host "ERROR: Python FastAPI service (Port 8000) is not running!" -ForegroundColor Red
    exit 1
}
...
try:
    ...
    with conn.cursor() as c:
        # 清除上一轮的脏测试残余数据，保证幂等性
        c.execute("DELETE FROM portfolio_transaction WHERE portfolio_id='TEST_PORTFOLIO_001'")
...
$depResp = Invoke-RestMethod -Uri "http://127.0.0.1:8080/api/v1/portfolios/TEST_PORTFOLIO_001/transactions" -Method Post -Body $depositBody -Headers $headers
...
$buyResp = Invoke-RestMethod -Uri "http://127.0.0.1:8080/api/v1/portfolios/TEST_PORTFOLIO_001/transactions" -Method Post -Body $buyBody -Headers $headers
Write-Host "   -> BUY 100 AAPL @ 5.00" -ForegroundColor DarkGray

$portInfo = Invoke-RestMethod -Uri "http://127.0.0.1:8080/api/v1/portfolios/TEST_PORTFOLIO_001" -Method Get
...
$priceSnapshots = Invoke-RestMethod -Uri "http://127.0.0.1:8000/api/v1/market/prices?tickers=AAPL" -Method Get
...
try {
    $summary = Invoke-RestMethod -Uri "http://127.0.0.1:8080/api/v1/portfolios/TEST_PORTFOLIO_001/summary" -Method Get
